...
- **`/vs-rfc-research`** — when the grill uncovered tradeoffs, competing approaches, or prior art worth formalizing. Fits technical decisions (architecture, library selection, system design) where a documented proposal will outlive this conversation.
...
**Prev:** `/vs-rfc-research` (proposal to stress-test) | user's plan or idea
**Next:** `/vs-autopilot` (execute the grilled plan) | fix the plan and `/vs-grill-me` again
