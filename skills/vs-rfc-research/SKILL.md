...
3. Suggest `/vs-grill-me` to stress-test the proposal before committing to it — the RFC is a plan, and plans benefit from adversarial review
...
**Prev:** `/vs-octocode-research` (deep-dive research that feeds the RFC)
**Next:** `/vs-grill-me` (stress-test the RFC) | `/vs-autopilot` (implement the approved RFC)
